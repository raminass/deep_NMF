...
for train_index, test_index in kf.split(V.T):
    ...
    for lam in range(3):
        ...
        _, op1_error, _, _ = util.train_unsupervised_opt1(
            data, layers, n_iter*10, n_components, l_1=L1, l_2=L2
        )
        ###### MU ################
        # train
...
fig_op1_brca_cv[f"dnmf_0"] = lst["dnmf_0"]
fig_op1_brca_cv[f"dnmf_1"] = lst["dnmf_1"]
fig_op1_brca_cv[f"dnmf_2"] = lst["dnmf_2"]
fig_op1_brca_cv[f"op1_0"] = lst["op1_0"]
fig_op1_brca_cv[f"op1_1"] = lst["op1_1"]
fig_op1_brca_cv[f"op1_2"] = lst["op1_2"]
fig_op1_brca_cv[f"mu_0"] = lst["mu_0"]
fig_op1_brca_cv[f"mu_1"] = lst["mu_1"]
fig_op1_brca_cv[f"mu_2"] = lst["mu_2"]
